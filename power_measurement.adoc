...
be provided as has been agreed. Failure to do so, will result in a invalid
submission.

=== SUT and Power Analyzer to remain unchanged from submission to results

The SUT (system under test) and the Power Analyzer remain unchanged
from the time of the submission until the results are published. This is to
ensure that in case there are audits, it is easiest to replicate the results
